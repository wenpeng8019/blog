...
* 关于 git 的用户和权限。千万要注意，git 的用户和权限并不是针对整个 git 的，甚至不是针对某个 branch 的，而是针对每次 commit 的。在每次 commit 时，git 都会给本次 commit 绑定一个用户身份（包括 user/email），用于标识该 commit 是谁提交的。当 git 用于本地时，这个用户身份并没有实际用处。但在和远程 git 库同步时，这个身份就会至关重要，因为远程 git 库默认是基于多用户并行同步的。而远程同步又是以 commit 为单位的，因此，每个 commit 在 push 到远程时，远程默认都会验证 commit 的身份。
  ...
  而**这里的关键在于**，我们可以随时重置（调整）proj 的身份邮箱，这就是说，多次 commit 版本的身份可能不同。例如：我们初始使用的是默认的私人邮箱，提交了两个 commit 版本。之后调整为公司邮箱，再提交两个 commit 版本。此时，前两个 commit 版本的身份就是私人邮箱，后面两个 commit 的身份则是公司邮箱。之后，当我们将本地的版本 push 到公司的远程 git 库时，前两个 commit 的提交就会报错，即“身份无效”。
...
* HEAD/Stage&Index/stash
  * HEAD 是一个指针，表示当前版本状态的指向。重置版本，其实就是设置 HEAD 指针的指向。然而，HEAD 的本质其实是一个参数，用于指定 git 命令要操作的目标 branch 对象。只是在 git 项目的 conf 中会默认设置好 HEAD 的值，并将其作为“当前 branch”，因此，默认的 git 命令一般都无需再指定该参数。
  * Staging Area 被称为暂存区。它其实是一个逻辑层面的抽象概念。对应的是 Working Area 工作区。工作区是项目实打实（当前操作中的版本）的全文件目录，也就是 proj 根目录。其下的 .git 子目录就是 git 库，它保存了 git 的配置、各个分支库、历史版本等一切信息。工作区的文件都是此刻的最新更新结果，但在 git 看来，它们都是基于最后一个（当前）commit 版本的改动，而在下次 commit 前，这些改动就处于“暂存”的状态，它其实是工作区当前文件和上一个 commit 版本文件的差异信息。而 Index 就是记录这些差异信息的技术实现，也就是数据差异索引。git 将 Index 记录的数据，抽象成了暂存区的概念，用来表示“暂存的数据版本”，在下次提交时，Index 的数据就会直接转换为 commit 版本。
  * stash 涉及到 **git 操作的一个禁忌**。就是在进行版本切换前，包括版本回退（或重置）、以及（更多数情况下的）分支切换，要确保暂存区是“空的”。因为版本切换会重构项目目录下的所有文件结构。这会导致没有被提交的数据更新被“清洗”覆盖。然而，我们在工作到一半时，是不会去提交一个“还未完成”的版本的。此时，就需要将这完成了一半的数据状态存储到一个特殊区域，即 stash （贮藏区）。
* fetch/FETCH_HEAD
